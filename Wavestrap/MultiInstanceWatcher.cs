namespace Wavestrap
{
    internal static class MultiInstanceWatcher
    {
        private static int GetOpenProcessesCount()
        {
            ...
            try
            {
                // prevent any possible race conditions by checking for Wavestrap processes too
                int count = Process.GetProcesses().Count(x => x.ProcessName is "RobloxPlayerBeta" or "Wavestrap");
